...
public final class SlimeFlowEditPanel {
	...
	private static long copiedFeedbackUntil = 0;
	...
	private static void renderHeader(Minecraft client, GuiGraphicsExtractor graphics, int x, int y) {
		...
		boolean justCopied = System.currentTimeMillis() < copiedFeedbackUntil;
		SlimeFlowUi.drawButton(client, graphics, x + EDIT_W - 84, y + 4, 32, 12, justCopied ? "Copied" : "Cpy", justCopied ? SlimeFlowTheme.GREEN : SlimeFlowTheme.BLUE);
	...
	public static boolean click(Minecraft client, int screenWidth, int screenHeight, double mouseX, double mouseY) {
		...
		if (SlimeFlowUi.inside(mouseX, mouseY, x + EDIT_W - 84, y + 4, 32, 12)) {
			...
			if (copied) {
				copiedFeedbackUntil = System.currentTimeMillis() + 1200;
			}
	...
	public static boolean pickHoveredSlot(Minecraft client) {
		...
		if (SlimeFlowState.pickMode == SlimeFlowState.PickMode.MOVE_TO) {
			...
			if (profile.rows.size() < MAX_ROWS && canAddMove()) {
				...
				SlimeFlowState.draftFromSlot = -1;
				SlimeFlowState.draftToSlot = -1;
				SlimeFlowState.pickMode = SlimeFlowState.PickMode.MOVE_FROM;
			}
		...
		if (SlimeFlowState.pickMode == SlimeFlowState.PickMode.MULTI_TO) {
			...
			if (profile.rows.size() < MAX_ROWS && canAddMulti()) {
				profile.rows.add(SlimeFlowProfile.Row.multi(
				...
				SlimeFlowState.draftMultiItemNames.clear();
				SlimeFlowState.draftMultiTargetSlot = -1;
				SlimeFlowState.pickMode = SlimeFlowState.PickMode.MULTI_ITEM;
			}
	...
	public static String[] pickListLines() {
		...
		if (SlimeFlowState.pickMode == SlimeFlowState.PickMode.MOVE_FROM || SlimeFlowState.pickMode == SlimeFlowState.PickMode.MOVE_TO) {
			...
			line2 = "Amount: " + SlimeFlowState.draftAmount + addedRowsText(profile);
			...
			line2 = "Button: " + buttonName(SlimeFlowState.draftClickButton) + "  Times: " + SlimeFlowState.draftClickTimes + addedRowsText(profile);
			...
			line3 = "Amount: " + SlimeFlowState.draftMultiAmount + addedRowsText(profile);
		} else if (SlimeFlowState.pickMode == SlimeFlowState.PickMode.OUTPUT_SLOT) {
			line1 = slotShort("Output", SlimeFlowState.draftOutputSlot);
			line2 = (profile == null || profile.outputCollectLoop ? "Mode: Loop collect" : "Mode: Once") + addedRowsText(profile);
		} else if (SlimeFlowState.pickMode == SlimeFlowState.PickMode.DROP_ITEM) {
			line1 = itemShort(SlimeFlowState.draftDropItemName);
			line2 = dropScopeLabel(SlimeFlowState.draftDropScope) + "  " + (SlimeFlowState.draftDropAmount <= 0 ? "All" : "A" + SlimeFlowState.draftDropAmount) + addedRowsText(profile);
